...
namespace WinTenDev.ZiziBot.App.Handlers.Core
{
    ...
    public abstract class ZiziEventHandler : BotEventHandler
    {
        ...
        )
        {
            ...
            {
                Log.Error(e, "Message: {V}", e.Message);
            }
        ...
        {
            var message = new HtmlString().Text(messageStr);
            return await EditMessageTextAsync(message, replyMarkup, parseMode, targetMessageId, chatId, addStamp);
        }
        ...
        )
        {
            ...
            if (targetMessageId == 0 && SentMessage != null) targetMessageId = SentMessage.MessageId;
            ...
            Log.Debug("Editing message in '{Chat} for MessageId '{TargetMessageId}'", chatId, targetMessageId);
        ...
        /// <summary>
        /// This method is used to promote or demote given userId on chatId target
        /// </summary>
        /// <param name="userId"></param>
        /// <param name="promote"></param>
        /// <returns></returns>
        protected async Task<TelegramResult> PromoteMember(long userId, bool promote)
        {
        ...
        protected async Task DeleteMessageAsync(int messageId = 0, int delaySecond = 0)
        {
            if (SentMessage == null)
            ...
            Log.Information("Deleting message on ChatId '{ChatId}' with messageId '{MessageId}'", ChatId, messageId);
        ...
        protected bool IsMessageOlderThan(TimeSpan time)
        {
            ...
            Log.Debug("On ChatId '{ChatId}', MessageId '{MessageId}' is older than time '{Time}'? {IsOld}",
            Chat.Id, messageId, time, isOld);
