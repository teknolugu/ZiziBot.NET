...
namespace WinTenDev.ZiziBot.App.Handlers.Tags
{
    ...
    public class TagHandler : ZiziEventHandler
    {
        ...
        private async Task ProcessingSave(CloudTag param, bool reTag)
        {
            if (reTag)
            {
            ...
            {
                await SendMessageTextAsync($"Sepertinya tag '{param.Tag}' sudah ada. " +
    ...
    public class TagParser : IRawParameterParser<CloudTag>
    {
        ...
        /// <returns></returns>
        public bool TryGetValueByRawUpdate(Update update, out CloudTag result)
        {
