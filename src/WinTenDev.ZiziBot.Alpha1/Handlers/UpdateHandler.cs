...
using WinTenDev.ZiziBot.Alpha1.Handlers.Modules;
...
namespace WinTenDev.ZiziBot.Alpha1.Handlers
{
    ...
    public class UpdateHandler : ZiziEventHandler
    {
        ...
        private async Task<bool> CheckScanMessageAsync()
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ...
                    nameof(CheckScanMessageAsync).Humanize()
                );
                return false;
            }
        ...
        private async Task<bool> CheckChatRestricted()
        {
            ...
            var checkRestriction = await _chatRestrictionModule.CheckRestriction(
                ChatId,
                async result => {
        ...
        private async Task<bool> CheckMention()
        {
            ...
            if (messageEntities != null)
            {
                ...
                {
                    anyMention = true;
                    messageMention.ForEach(
                        async (
                            target,
                            i
                        ) => {
                            // await SendMessageTextAsync(message: target, messageStr: "Lorem ipsum dolor sit amet");
                        }
                    );
                }
            }
        ...
        private async Task<bool> CheckFireMessage()
        {
            ...
            if (message == null)
            {
                ...
                _logger.LogInformation(
                    "MessageId '{MessageId}' on ChatID '{ChatId}' is type {Type}",
                    messageId,
                    ChatId,
                    RawUpdate.Type
                );
            ...
            {
                _logger.LogDebug(
                    "No Message Text/Caption on ChatId '{ChatId}' for MessageId '{MessageId}'",
                    ChatId,
                    message.MessageId
                );
